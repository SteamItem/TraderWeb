...
<script>
  ...
  export default {
    ...
    computed: {
      pageTitle () {
        if (this.wishlist) {
          return `${this.wishlist.name} - Wishlist Item Manager`
        } else {
          return `Wishlist Item Manager`
        }
      },
      formTitle () {
    ...
    methods: {
      ...
      async findWishlist() {
        ...
        var response = await axios.get(`${process.env.VUE_APP_API_URL}/wishlist/${id}`);
        this.wishlist = response.data;
      },
      async findAll() {
        var id = this.$route.params.id;
        var response = await axios.get(`${process.env.VUE_APP_API_URL}/wishlistItems/${id}`);
      ...
      async deleteItem (item) {
        ...
        if (confirmed) {
          var id = this.$route.params.id;
          await axios.delete(`${process.env.VUE_APP_API_URL}/wishlistItems/${id}/${item._id}`);
          await this.findAll();
        }
      },
      close () {
      ...
      saveItem () {
        var id = this.$route.params.id;
        return axios.post(`${process.env.VUE_APP_API_URL}/wishlistItems/${id}`, {
