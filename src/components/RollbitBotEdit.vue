...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getBot() {
        var botId = this.$route.params.botId;
        var result = await axios.get(`/api/bot/rollbit/${botId}`, { headers: { Authorization: `Bearer ${this.token}` }});
      ...
      async createBot() {
        const result = await axios.post(`/api/bot/rollbit`, { name: this.bot.name }, { headers: { Authorization: `Bearer ${this.token}` }});
        const botId = result.data._id;
        this.$router.push({ path: `/RollbitBotEdit/${botId}`});
        this.bot = result.data;
        this.isAdd = false;
      },
      async updateBot() {
        let botId = this.$route.params.botId;
        return axios.put(`/api/bot/rollbit/${botId}`, {
          name: this.bot.name,
      ...
      async deleteBot() {
        ...
        if (confirmed) {
          await axios.delete(`/api/bot/rollbit/${botId}`, { headers: { Authorization: `Bearer ${this.token}` }});
          this.$router.push({ path: `/RollbitBots`});
        }
      },
      async findWishlistItems() {
        var botId = this.$route.params.botId;
        var response = await axios.get(`/api/wishlistItems/rollbit/${botId}`, { headers: { Authorization: `Bearer ${this.token}` }});
      ...
      async createWishlistItem () {
        var botId = this.$route.params.botId;
        return await axios.post(`/api/wishlistItems/rollbit/${botId}`, {
      ...
      async updateWishlistItem() {
        var botId = this.$route.params.botId;
        return await axios.put(`/api/wishlistItems/rollbit/${botId}/${this.wishlistItem._id}`, {
          name: this.wishlistItem.name,
          max_price: this.wishlistItem.max_price
        }, { headers: { Authorization: `Bearer ${this.token}` }});
      },
      async deleteWishlistItem (item) {
        ...
        if (confirmed) {
          await axios.delete(`/api/wishlistItems/rollbit/${botId}/${item._id}`, { headers: { Authorization: `Bearer ${this.token}` }});
          await this.findWishlistItems();
        }
      },
      async signIn () {
        ...
        axios.post(`/api/bot/rollbit/login/${botId}`, that.steamLogin, { headers: { Authorization: `Bearer ${this.token}` }}).then(() => {
