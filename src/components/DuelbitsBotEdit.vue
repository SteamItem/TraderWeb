...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getBot() {
        var botId = this.$route.params.botId;
        var result = await axios.get(`/api/bot/duelbits/${botId}`, { headers: { Authorization: `Bearer ${this.token}` }});
      ...
      async createBot() {
        const result = await axios.post(`/api/bot/duelbits`, { name: this.bot.name }, { headers: { Authorization: `Bearer ${this.token}` }});
      ...
      async updateBot() {
        ...
        return axios.put(`/api/bot/duelbits/${botId}`, {
          name: this.bot.name,
      ...
      async deleteBot() {
        ...
        if (confirmed) {
          await axios.delete(`/api/bot/duelbits/${botId}`, { headers: { Authorization: `Bearer ${this.token}` }});
          this.$router.push({ path: `/DuelbitsBots`});
        }
      },
      async findWishlistItems() {
        var botId = this.$route.params.botId;
        var response = await axios.get(`/api/wishlistItems/duelbits/${botId}`, { headers: { Authorization: `Bearer ${this.token}` }});
      ...
      async createWishlistItem () {
        var botId = this.$route.params.botId;
        return await axios.post(`/api/wishlistItems/duelbits/${botId}`, {
      ...
      async updateWishlistItem() {
        var botId = this.$route.params.botId;
        return await axios.put(`/api/wishlistItems/duelbits/${botId}/${this.wishlistItem._id}`, {
          name: this.wishlistItem.name,
          max_price: this.wishlistItem.max_price
        }, { headers: { Authorization: `Bearer ${this.token}` }});
      },
      async deleteWishlistItem (item) {
        ...
        if (confirmed) {
          await axios.delete(`/api/wishlistItems/duelbits/${botId}/${item._id}`, { headers: { Authorization: `Bearer ${this.token}` }});
          await this.findWishlistItems();
        }
      },
      async signIn () {
        ...
        axios.post(`/api/bot/duelbits/login/${botId}`, that.steamLogin, { headers: { Authorization: `Bearer ${this.token}` }}).then(() => {
